...
- **`app.harness(provider="opencode")` is available by default.** When a reasoner genuinely needs a coding agent in the loop — file I/O, shell commands, multi-step coding tasks — you can use it without any extra setup.
...
**When to use `app.harness(provider="opencode")`:**

Use it when the task genuinely requires a **real coding agent** — reading/writing files on disk, running shell commands, navigating a codebase, or performing multi-step coding work that `app.ai(tools=[...])` cannot do. Examples:
- A reasoner that fixes a failing test by reading the test file, understanding the error, and editing the code
- A reasoner that generates a report by navigating a directory tree and synthesizing file contents
- A reasoner that runs shell commands to inspect system state as part of its analysis

**When NOT to use `app.harness()`:**

- For in-process stateful LLM reasoning over a document → use `app.ai(tools=[...])`
- For embarrassingly-parallel chunked analysis → use a `@app.reasoner()` that loops `app.ai()` with `asyncio.gather`
- For classification, routing, structured extraction → use plain `app.ai(schema=...)`

`app.harness()` is heavyweight — it spawns an entire coding-agent subprocess. Use it when you need the power, not as a substitute for `app.ai(tools=[...])`.
...
| "I need stateful tool-using → `app.harness()`" | If you need file I/O or shell access, yes — use `app.harness(provider="opencode")` (pre-installed). If you just need multi-turn LLM reasoning, use `app.ai(tools=[...])` instead — it's lighter |
