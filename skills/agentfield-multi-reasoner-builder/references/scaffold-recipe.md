...
- [ ] Every primitive passes the intelligence test: `@app.skill()` for anything programmatic, `app.ai()` for judgment, `app.harness()` only when the LLM's reasoning autonomously drives which actions to take
