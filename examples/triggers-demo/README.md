...
Run AgentField with a sample agent that handles three real-world webhook
patterns — **Stripe payments**, **GitHub pull requests**, and a **cron
schedule** — and watch the events flow through the UI live.

The agent is fully deterministic (no LLM calls), so this demo is the
...
| Reasoner | Source | Fires on |
|---|---|---|
| `handle_payment` | `stripe` | `payment_intent.succeeded` |
...
When you load `http://localhost:8080/triggers`, you'll see three trigger
rows — Stripe, GitHub, cron — each with a public URL, an enabled toggle, a
"code" badge, and the source code location they were declared at
(`agent.py:NN`).
...
### 4. `/runs` — execution rows tagged with their trigger

When the agent finishes processing an event, the resulting run shows up in
`/runs` (or the existing executions/workflows surface). Rows whose run was
kicked off by an inbound event show a small `↪ Stripe` / `↪ GitHub` /
`↪ Cron` badge next to the run identifier.

### 5. `/runs/:id` — run detail with the webhook input
...
```bash
STRIPE_DEMO_SECRET=whsec_xxx GITHUB_DEMO_SECRET=ghsecret_xxx \
  docker compose up --build -d
```
